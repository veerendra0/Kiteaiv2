...
def get_user_delays():
    try:
        min_delay = float(input("Enter minimum delay (in seconds): "))
        max_delay = float(input("Enter maximum delay (in seconds): "))
        if min_delay > max_delay:
            min_delay, max_delay = max_delay, min_delay
        return min_delay, max_delay
    except ValueError:
        print("Invalid input. Using default delay 3-7 seconds.")
        return 3, 7

def random_delay(task_name, min_delay, max_delay):
    delay = random.uniform(min_delay, max_delay)
    print(f"[⏳] {task_name} will delay for {delay:.2f} seconds...")
    time.sleep(delay)

# Prompt once when script starts
min_delay, max_delay = get_user_delays()
...
class KiteAi:
    ...
    random_delay("Agent Interaction", min_delay, max_delay)
