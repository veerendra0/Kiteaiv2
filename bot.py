...
class KiteAi:
    ...
    def print_question(self):
        ...
        while True:
            try:
                self.min_agent_delay = int(input(f"{Fore.YELLOW + Style.BRIGHT}Min Agent Delay (seconds)? -> {Style.RESET_ALL}").strip())
                self.max_agent_delay = int(input(f"{Fore.YELLOW + Style.BRIGHT}Max Agent Delay (seconds)? -> {Style.RESET_ALL}").strip())
                if self.max_agent_delay >= self.min_agent_delay:
                    break
                else:
                    print(f"{Fore.RED + Style.BRIGHT}Max must be >= Min.{Style.RESET_ALL}")
            except ValueError:
                print(f"{Fore.RED + Style.BRIGHT}Enter valid numbers.{Style.RESET_ALL}")

        while True:
            try:
                self.min_bridge_delay = int(input(f"{Fore.YELLOW + Style.BRIGHT}Min Bridge Delay (seconds)? -> {Style.RESET_ALL}").strip())
                self.max_bridge_delay = int(input(f"{Fore.YELLOW + Style.BRIGHT}Max Bridge Delay (seconds)? -> {Style.RESET_ALL}").strip())
                if self.max_bridge_delay >= self.min_bridge_delay:
                    break
                else:
                    print(f"{Fore.RED + Style.BRIGHT}Max must be >= Min.{Style.RESET_ALL}")
            except ValueError:
    ...
    async def random_delay(self, min_delay: int, max_delay: int, label: str = "Delay"):
        ...
        for remaining in range(delay, 0, -1):
            print(
                ...
                f"{Fore.WHITE + Style.BRIGHT} {remaining} sec remaining...{Style.RESET_ALL}",
                end="\r",
                flush=True
            )
            await asyncio.sleep(1)
...
async def solve_recaptcha(self, site_key: str, page_url: str, retries=5):
    ...
    async def process_option_5(self, address: str, sa_address: str, use_proxy: bool):
        ...
        for i in range(self.chat_count):
            ...
            await self.random_delay(self.min_agent_delay, self.max_agent_delay, "Agent Delay")
    ...
    async def process_option_6(self, account: str, address: str, use_proxy: bool):
        ...
        for i in range(self.bridge_count):
            ...
            await self.random_delay(self.min_bridge_delay, self.max_bridge_delay, "Bridge Delay")
